...
public:
    Produit(const string& name, const string& unit="") : name_(name), unit_(unit) {}
    string getNom() const {return name_;}
    string getUnite() const {return unit_;}
...
public:
    Ingredient(const Produit& produit, const double quantity) : produit_(produit), quantity_(quantity) {}
...
private:
    string name_;
    double quantity_coeff_;
    vector<Ingredient*> ing_list_;
public:
    Recette(const string& name, const double quantity_coeff)
        : name_(name), quantity_coeff_(quantity_coeff), ing_list_({}) {}
    void ajouter(const Produit& produit, double quantity) {
